...
async function topGainersRoutes(fastify, options) {
  const prisma = prismaClient.getClient();

  // Smart cache keys for top gainers
  const CACHE_KEYS = {
    ALL: 'top_gainers_all'
  };
  ...
  // Get all top gainers with smart caching
  fastify.get('/', async (request, reply) => {
    try {
      ...
      const [data, total] = await Promise.all([
        prisma.topGainer.findMany({
          orderBy: { rank: 'asc' },
          skip: (page - 1) * limit,
          take: limit
        }),
        prisma.topGainer.count()
      ]);
...
module.exports = topGainersRoutes;
