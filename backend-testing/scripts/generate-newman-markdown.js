...
function generateNewmanMarkdown(
  inputPath = path.join(ROOT, 'reports/newman-report.json'),
  outputPath = path.join(ROOT, 'reports/newman-report.md')
) {
  if (!fs.existsSync(inputPath)) {
    throw new Error(`Missing Newman JSON report at ${inputPath}`);
  }

  const templateDir = path.join(ROOT, 'reports', 'templates');
  ...
  const testCaseContent = replaceTokens(testCaseTemplate, {
    ...
    APP_VERSION: process.env.TEST_TARGET_VERSION || process.env.GITHUB_SHA?.slice(0, 8) || 'N/A',
    ...
    OS: process.platform,
    SERVER: appUrl,
    PRECONDITIONS: `Postman collection and environment configured; Newman installed; Total assertions: ${totalAssertions}.`,
  ...
  const bugSections = failures.map((failure, index) => {
    ...
    return replaceTokens(bugTemplate, {
      ...
      PRECONDITION: 'Collection executed through Newman.',
...
if (require.main === module) {
  const inputPath = process.argv[2] || path.join(ROOT, 'reports/newman-report.json');
  const outputPath = process.argv[3] || path.join(ROOT, 'reports/newman-report.md');
