...
| Field | Value |
|-------|--------|
| **ID Number#** | BUG-PWD-004 |
| **Title** | STC-PWD-CHANGE-001B: POST — Change password success |
...
| Field | Value |
|-------|--------|
| **Summary** | STC-PWD-CHANGE-001B: STC-PWD-CHANGE-001/B \| Change password success — assertion failure (HTTP 401 Unauthorized). expected response to have status code 200 but got 401 |
...
| Field | Value |
|-------|--------|
| **Precondition** | STC STC-PWD-CHANGE-001/B mapped to this request; authenticated context per collection (tokens as saved in environment). |
| **Steps to Reproduce** | 1. Configure environment: base URL https://identity.physio.agregatech.com.<br>2. In Postman/Newman, open the request named "STC-PWD-CHANGE-001/B \| Change password success".<br>3. Send POST to https://identity.physio.agregatech.com/api/v1/kine/auth/change-password with the collection's body and headers.<br>4. Observe HTTP status, response body, and Newman test assertions. |
| **Expected Result** | Cas nominal validé selon les AC du ticket : • POST /auth/change-password : ancien + nouveau password • Application robustesse password • PATCH /me : whitelist stricte (nom, téléphone, etc.) Align API with spreadsheet specification or adjust Newman tests after agreement. |
| **Actual Result** | 401 Unauthorized in 153 ms. 1 failed, 1 passed. expected response to have status code 200 but got 401 — expected response to have status code 200 but got 401 |
...
| Field | Value |
|-------|--------|
| **Notes** | expected response to have status code 200 but got 401 |
